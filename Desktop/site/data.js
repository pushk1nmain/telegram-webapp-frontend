...
// Проверка доступа через Telegram (ОТКЛЮЧЕНА для отладки)
function checkTelegramAccess() {
    // ВСЕГДА разрешаем доступ для отладки
    console.log('🛠️ Проверка Telegram отключена для отладки - доступ разрешен');
    return true;
    
    // Закомментированная оригинальная проверка:
    /*
